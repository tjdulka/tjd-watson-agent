...
module.exports = function (app) {
  ...
  var seedUsers = [{ username: 'watson', email: 'watson@ibm.com', password: 'p@ssw0rd' }, { username: 'admin', email: 'admin@ibm.com', password: '@dm1n' }];
  ...
  seedUsers.forEach(function (seedUser) {
    try {
      ...
      Auth.findOne({ where: { username: seedUser.username } }, function (err, user) {
          ...
          if (role) {
            ...
            Role.create(seedRole, function (err, role) {
              ...
              role.principals.create({
                principalType: RoleMapping.USER, principalId: seedUsers[1].id }, function (err, principal) {
                if (err) throw err;
